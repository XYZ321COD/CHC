...
def tree_loss(tree_output1, tree_output2, batch_size, mask_for_level, mean_of_probs_per_level_per_epoch, args):
        ## TREE LOSS
    ...
    for level in range(1, args.level_number + 1):
        ...
        if level == args.level_number:
            mean_of_probs_per_level_per_epoch[args.level_number] += torch.mean(prob_features, dim=0)
...
def regularization_loss(tree_output1, tree_output2,  masks_for_level, args):
    out_tree = torch.cat([tree_output1, tree_output2], dim=0)
